...
DOCKER_COMPOSE_PHP_FPM_EXEC = ${DOCKER_COMPOSE} exec choco-php-fpm
...
console:
	if ! ${DOCKER_COMPOSE} ps | grep -q choco-php-fpm; then make up; fi
	${DOCKER_COMPOSE_PHP_FPM_EXEC} bash

create_network:
	docker network create --subnet 172.18.4.0/24 choco_network >/dev/null 2>&1 || true
