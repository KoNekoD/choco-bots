...
final readonly class PromoteReputationCommandHandler
    implements CommandHandlerInterface
{
    ...
    public function __invoke(
        PromoteReputationCommand $command
    ): ChatClientResultDTO {
        try {
            ...
            if ($command->data->getUpdateMessageFrom()->getSourceUserId(
                ) === $repliedUser->getSourceUserId()) {
                throw new ChatMemberException(
                    $this->translator->trans(
                        'ChatMemberReputationSelfPromotion'
                    )
                );
            }
            ...
            for ($i = 0; $i < $command->getPromoteReputationValue(); $i++) {
                try {
                    $repliedChatMember->promoteReputation($who);
